    ...
    return [
        {
            ...
            columns: [
                ...
                },
                {
                    ...
                    data: (pool)=> {
                        ...
                        return tableUtil.initMysqlServerFilter(pool, sqlCommand);
                    }
                },
            ...
            read: ()=> {
                ...
                let groupStr = tableUtil.group(["server", "day"]);
                let whereStr1 = tableUtil.where([
                    tableUtil.condition.rangeDate("date", "day"),
                    tableUtil.condition.optionalSelectNum("serverid", "server"),
                    tableUtil.condition.notEqual("serverid", 0)
                ]);
                let whereStr2 = tableUtil.where([
                    tableUtil.condition.rangeDate("date(time)", "day"),
                    tableUtil.condition.optionalSelectNum("serverid", "server"),
                    tableUtil.condition.notEqual("serverid", 0)
                ]);
                let buildJoinCondition = (server, table)=> {
                    if (server.length != 0) {
                        return `online.server = ${table}.server and online.day = ${table}.day`;
                    } else {
                        return `online.day = ${table}.day`;
                    }
                };

                let sqlCommand = `
                    select online.day as day,${tableUtil.column.optionalSelect("server", "server", "online")}new.dnu as dnu,round(charge.chargeNewRoleNum/new.dnu,2) dnuRate,
                    ...
                    on ${buildJoinCondition(server, "new")}
                    left join 
                    ...
                    order by online.day,online.server
                    `;
                return sqlCommand;
            },
            readCheck: ()=> {
                return tableUtil.check.rangeDay("day") && tableUtil.check.select("server");
            }
        },
        {
            id: "resource",
            ...
            read: ()=> {
                ...
                let whereArr = [
                    ...
                    tableUtil.condition.notEqual("serverId", 0),
                    tableUtil.condition.rangeDate("date", "day"),
                    tableUtil.condition.optionalSelectNum("vipLevel", "vipLevel"),
                    tableUtil.condition.notEqual(valueStr, 0)
                ];
                let whereStr = tableUtil.where(whereArr);
                let groupStr = tableUtil.group(["server", "day", "source"]);
            ...
            readCheck: ()=> {
                return tableUtil.check.rangeDay("day") && tableUtil.check.select("server") && tableUtil.check.select("vipLevel");
            },
            readMap: (data)=> {
                let sourceArr = [
        ...
        },
        {
            ...
            readMap: (data)=> {
                ...
                if (server.length != 0) {
                    ...
                    data.forEach(d=> {
                        ...
                        if (findJson != undefined && findJson.num <= 29) {
                            findJson.num++;
            ...
            readCheck: ()=> {
                return tableUtil.check.select("server");
            }
        },
        {
            id: "charge",
            ...
            read: ()=> {
                let whereStr = tableUtil.where([
                    tableUtil.condition.optionalInputStr("js_id", "role"),
                    tableUtil.condition.optionalInputStr("orderid", "orderId"),
                    tableUtil.condition.optionalSelectStr("status", "chargeStatus"),
                    tableUtil.condition.rangeDate("time", "second"),
                    tableUtil.condition.optionalSelectNum("serverid", "server"),
                    tableUtil.condition.notEqual("serverid", 0)
                ]);
            ...
            readCheck: ()=> {
                return tableUtil.check.optionalRegex("role", /^\d+$/) && tableUtil.check.optionalRegex("orderId", /^\d+$/) &&
                    tableUtil.check.select("chargeStatus") && tableUtil.check.rangeSecond("second") && tableUtil.check.select("server");
            }
        },
        {
            id: "cost",
            ...
            readCheck: ()=> {
                return tableUtil.check.optionalRegex("role", /^\d+$/) && tableUtil.check.rangeSecond("second") && tableUtil.check.select("server");
            }
        },
        {
            id: "stamina",
            ...
            readCheck: ()=> {
                return tableUtil.check.rangeSecond("second") && tableUtil.check.select("server") && tableUtil.check.optionalRegex("role", /^\d+$/);
            }
        },
        {
            id: "produce",
        ...
        },
        {
            ...
            extraFilter: [
                ...
                {id: "pk", name: "角色pk", type: "integer", placeholder: "角色pk,必填", required: true},
                {
                    ...
                    data: [
                        ...
                        "充值发货",
                        "秘境探宝普通-战斗结束", "秘境探宝普通-扫荡", "秘境探宝普通-随机宝箱", "秘境探宝普通-重置地图", "装备鉴定", "竞技场-获得积分", "竞技场-挑战",
                        "竞技场-清除挑战CD", "竞技场-购买挑战次数", "竞技场-奖励兑换", "抽英雄碎片-金币", "抽英雄碎片-钻石", "挖矿-开始挖矿", "挖矿-收获矿石",
                        "挖矿-开辟矿洞", "挖矿-获得随机掉落", "挖矿-一键挖矿", "挖矿-一键收矿", "宝石城-按钮2", "宝石城-按钮1", "宝石城-按钮3", "军团-创建军团",
                        "宠物-拆分宠物", "宠物-吃经验升级", "拍卖行-购买物品", "拍卖行-上架物品", "拍卖行-到期下架", "拍卖行-物品售出", "英雄-吃书升级",
                        "英雄-吃书升阶", "英雄-拆分", "背包-出售物品", "背包-使用物品", "卡带-抽一次", "卡带-金币10连抽", "卡带-金币50连抽", "卡带-一键钻石",
                        "卡带-吞噬", "邮件-邮件信息", "邮件-查看邮件", "邮件-领取附件", "邮件-删除邮件", "招财猫-招财", "卡带-洗练", "卡带-合成", "宝石-一键合成",
                        "送花-送花", "送花-兑换", "宝石-合成", "宝石-拆分", "顶水果-倍数1", "顶水果-倍数2", "顶水果-倍数3", "顶水果-大箱子", "宝物-升星", "宝物-神铸",
                        "装备-精炼", "精英副本-扫荡", "精英副本-结束战斗", "精英副本-购买扫荡次数", "精英副本-开箱子", "军团组队副本-副本开启", "军团组队副本-结束战斗",
                        "军团组队副本-触发事件", "军团-捐献", "普通副本-扫荡", "普通副本-结束战斗", "普通副本-开箱子", "任务礼包-得到奖励", "体力-非自动回复",
                        "体力-自动回复", "体力-购买体力", "签到-本日签到", "签到-领取累计签到奖励", "等级礼包商城-购买物品", "VIP礼包商城-购买物品", "资源争夺-战斗",
                        "资源争夺-复仇", "世界BOSS-复活", "世界BOSS-强化", "普通狩猎场-消耗子弹", "普通狩猎场-击中特殊怪物", "普通狩猎场-买子弹", "普通狩猎场-击中彩蛋怪",
                        "普通狩猎场-领取任务奖励", "普通狩猎场-击杀宝箱怪", "队伍等级礼包-领取", "超人礼包-领取", "7日登陆-领取", "自由组队-战斗结算", "自由组队-触发事件",
                        "成就-领取奖励", "试炼神殿-战斗结果", "军团BOSS-召唤BOSS", "军团普通副本-战斗结束", "军团普通副本-扫荡", "军团技能-使用卷轴", "军团技能-交换技能",
                        "循环活动-领取奖励", "日常活动-领取奖励", "首冲活动-领取奖励", "军团材料收集-刷新任务", "军团材料收集-领取奖励", "军团老虎机-改运",
                        "军团老虎机-领取奖励", "军团猜英雄-领取奖励", "军团工资-领取奖励", "月卡礼包-购买", "月卡礼包-领取奖励", "成长基金-购买", "成长基金-领取奖励",
                        "折扣商城-购买", "英雄兑换", "奴隶屋-领取互动奖励", "钻石狩猎场-消耗子弹", "钻石狩猎场-击中特殊怪物", "秘境探宝精英-战斗结束", "秘境探宝精英-扫荡",
                        "秘境探宝精英-重置", "转轮活动-开启转轮", "怪物攻城-复活", "怪物攻城-击杀奖励", "怪物攻城-强化", "元素塔-战斗结束", "元素塔-扫荡", "元素-强化",
                        "元素-分解", "元素-转化", "元素-进化", "队伍等级首冲-状态验证", "商城-购买超能币礼盒", "商城-超能币商城购买", "失落矿山-占领空矿",
                        "失落矿山-刷新矿点", "失落矿山-掠夺矿点", "失落矿山-邀请消息反馈", "失落矿山-手动收取资源", "失落矿山-购买掠夺次数", "天赋-技能加点",
                        "天赋-装备分解"
                    ]
                }
            ],
            read: ()=> {
                ...
                let moduleMap = [
                    ...
                    {id: "136-9401", name: "转轮活动-开启转轮"},
                    {id: "142-9607", name: "怪物攻城-复活"},
                    {id: "142-27", name: "怪物攻城-击杀奖励"},
                    {id: "142-9608", name: "怪物攻城-强化"},
                    ...
                    {id: "154-10225", name: "失落矿山-占领空矿"},
                    {id: "154-10202", name: "失落矿山-刷新矿点"},
                    {id: "154-10203", name: "失落矿山-掠夺矿点"},
                    {id: "154-10222", name: "失落矿山-邀请消息反馈"},
                    {id: "154-10216", name: "失落矿山-手动收取资源"},
                    {id: "154-10208", name: "失落矿山-购买掠夺次数"},
            ...
            readCheck: ()=> {
                let moduleRegex = new RegExp("^(角色战力|角色登出|教学|角色创建|等级提升|最后登录信息|costGVEnum|阵营选择|账号创建|角色登录|充值订单|充值发货|" +
                    "秘境探宝普通-战斗结束|秘境探宝普通-扫荡|秘境探宝普通-随机宝箱|秘境探宝普通-重置地图|装备鉴定|竞技场-获得积分|竞技场-挑战|竞技场-清除挑战CD|" +
                    "竞技场-购买挑战次数|竞技场-奖励兑换|抽英雄碎片-金币|抽英雄碎片-钻石|挖矿-开始挖矿|挖矿-收获矿石|挖矿-开辟矿洞|挖矿-获得随机掉落|挖矿-一键挖矿|" +
                    "挖矿-一键收矿|宝石城-按钮2|宝石城-按钮1|宝石城-按钮3|军团-创建军团|宠物-拆分宠物|宠物-吃经验升级|拍卖行-购买物品|拍卖行-上架物品|" +
                    "拍卖行-到期下架|拍卖行-物品售出|英雄-吃书升级|英雄-吃书升阶|英雄-拆分|背包-出售物品|背包-使用物品|卡带-抽一次|卡带-金币10连抽|卡带-金币50连抽|" +
                    "卡带-一键钻石|卡带-吞噬|邮件-邮件信息|邮件-查看邮件|邮件-领取附件|邮件-删除邮件|招财猫-招财|卡带-洗练|卡带-合成|宝石-一键合成|送花-送花|" +
                    "送花-兑换|宝石-合成|宝石-拆分|顶水果-倍数1|顶水果-倍数2|顶水果-倍数3|顶水果-大箱子|宝物-升星|宝物-神铸|装备-精炼|精英副本-扫荡|" +
                    "精英副本-结束战斗|精英副本-购买扫荡次数|精英副本-开箱子|军团组队副本-副本开启|军团组队副本-结束战斗|军团组队副本-触发事件|军团-捐献|" +
                    "普通副本-扫荡|普通副本-结束战斗|普通副本-开箱子|任务礼包-得到奖励|体力-非自动回复|体力-自动回复|体力-购买体力|签到-本日签到|" +
                    "签到-领取累计签到奖励|等级礼包商城-购买物品|VIP礼包商城-购买物品|资源争夺-战斗|资源争夺-复仇|世界BOSS-复活|世界BOSS-强化|" +
                    "普通狩猎场-消耗子弹|普通狩猎场-击中特殊怪物|普通狩猎场-买子弹|普通狩猎场-击中彩蛋怪|普通狩猎场-领取任务奖励|普通狩猎场-击杀宝箱怪|" +
                    "队伍等级礼包-领取|超人礼包-领取|7日登陆-领取|自由组队-战斗结算|自由组队-触发事件|成就-领取奖励|试炼神殿-战斗结果|军团BOSS-召唤BOSS|" +
                    "军团普通副本-战斗结束|军团普通副本-扫荡|军团技能-使用卷轴|军团技能-交换技能|循环活动-领取奖励|日常活动-领取奖励|首冲活动-领取奖励|" +
                    "军团材料收集-刷新任务|军团材料收集-领取奖励|军团老虎机-改运|军团老虎机-领取奖励|军团猜英雄-领取奖励|军团工资-领取奖励|月卡礼包-购买|" +
                    "月卡礼包-领取奖励|成长基金-购买|成长基金-领取奖励|折扣商城-购买|英雄兑换|奴隶屋-领取互动奖励|钻石狩猎场-消耗子弹|钻石狩猎场-击中特殊怪物|" +
                    "秘境探宝精英-战斗结束|秘境探宝精英-扫荡|秘境探宝精英-重置|转轮活动-开启转轮|怪物攻城-复活|怪物攻城-击杀奖励|怪物攻城-强化|元素塔-战斗结束|" +
                    "元素塔-扫荡|元素-强化|元素-分解|元素-转化|元素-进化|队伍等级首冲-状态验证|商城-购买超能币礼盒|商城-超能币商城购买|失落矿山-占领空矿|" +
                    "失落矿山-刷新矿点|失落矿山-掠夺矿点|失落矿山-邀请消息反馈|失落矿山-手动收取资源|失落矿山-购买掠夺次数|天赋-技能加点|天赋-装备分解)$");
                return tableUtil.check.rangeSecond("second") && tableUtil.check.regex("pk", /^\d+$/) && tableUtil.check.optionalRegex("server", /^s\d+$/)
                    && tableUtil.check.regex("module", moduleRegex);
            },
            readMap: (data)=> {
                let actionMap = [
                    {id: "juesezhanli", name: "角色战力"},
        ...
        },
        {
            ...
            columns: [
                ...
                {id: "av", name: "av", checked: false},
                {id: "chid", name: "chid", checked: false},
                {id: "et", name: "et", checked: false},
                {id: "mt", name: "mt", checked: false},
                {id: "nwt", name: "nwt", checked: false},
                {id: "ov", name: "ov", checked: false},
                {id: "pgn", name: "pgn", checked: false},
                {id: "tm", name: "tm", checked: false},
                {id: "type", name: "type", checked: false, tdStyle: {"white-space": "nowrap"}},
                {id: "total", name: "总计", checked: false},
                {id: "unknown", name: "未知", checked: false},
                {id: "java.lang.ClassCastException", name: "java.lang.ClassCastException", checked: false},
                {id: "Data name", name: "Data name", checked: false},
                {
                    id: "java.lang.ArithmeticException: divide by zero",
                    name: "java.lang.ArithmeticException: divide by zero",
                    checked: false
                },
                {id: "Invalid operation", name: "Invalid operation", checked: false},
                {id: "Invalid function", name: "Invalid function", checked: false},
                {id: "java.lang.IllegalArgumentException", name: "java.lang.IllegalArgumentException", checked: false},
                {id: "java.lang.StackOverflowError", name: "java.lang.StackOverflowError", checked: false},
                {id: "java.lang.NullPointerException", name: "java.lang.NullPointerException", checked: false},
                {id: "OutOfMemoryError", name: "OutOfMemoryError", checked: false},
                {id: "Don't load", name: "Don't load", checked: false},
                {id: "No yet bind struct", name: "No yet bind struct", checked: false},
                {
                    id: "java.lang.IndexOutOfBoundsException",
                    name: "java.lang.IndexOutOfBoundsException",
                    checked: false
                },
                {
                    id: "java.lang.ArrayIndexOutOfBoundsException",
                    name: "java.lang.ArrayIndexOutOfBoundsException",
                    checked: false
                },
                {id: "Duplicate space name dmspace", name: "Duplicate space name dmspace", checked: false},
                {id: "java.lang.RuntimeException", name: "java.lang.RuntimeException", checked: false},
            ],
            extraFilter: [
                {id: "display", name: "展示类型", checked: true, type: "radio", data: ["统计", "查询"]},
                {
                    id: "type", name: "崩溃类型", checked: true, type: "select",
                    data: [
                        {id: "java.lang.ClassCastException", name: "java.lang.ClassCastException", checked: true},
                        {id: "Data name", name: "Data name", checked: true},
                        {
                            id: "java.lang.ArithmeticException: divide by zero",
                            name: "java.lang.ArithmeticException: divide by zero",
                            checked: true
                        },
                        {id: "Invalid operation", name: "Invalid operation", checked: true},
                        {id: "Invalid function", name: "Invalid function", checked: true},
                        {id: "java.lang.IllegalArgumentException", name: "java.lang.IllegalArgumentException", checked: true},
                        {id: "java.lang.StackOverflowError", name: "java.lang.StackOverflowError", checked: true},
                        {id: "java.lang.NullPointerException", name: "java.lang.NullPointerException", checked: true},
                        {id: "OutOfMemoryError", name: "OutOfMemoryError", checked: true},
                        {id: "Don't load", name: "Don't load", checked: true},
                        {id: "No yet bind struct", name: "No yet bind struct", checked: true},
                        {
                            id: "java.lang.IndexOutOfBoundsException",
                            name: "java.lang.IndexOutOfBoundsException",
                            checked: true
                        ...
                        {id: "Duplicate space name dmspace", name: "Duplicate space name dmspace", checked: true},
                        {id: "java.lang.RuntimeException", name: "java.lang.RuntimeException", checked: true},
                        {id: "unknown", name: "未知", checked: true},
                    ]
                },
            ],
            read: ()=> {
                let typeArr = ["java.lang.ClassCastException", "Data name", "java.lang.ArithmeticException: divide by zero", "Invalid operation",
                    "Invalid function",
                    "java.lang.IllegalArgumentException", "java.lang.StackOverflowError", "java.lang.NullPointerException", "OutOfMemoryError",
                    "Don't load", "No yet bind struct", "java.lang.IndexOutOfBoundsException",
                    "java.lang.ArrayIndexOutOfBoundsException", "Duplicate space name dmspace", "java.lang.RuntimeException", "未知"];
                let typeStr = typeArr.map(d=> {
                    let asStr = d == "未知" ? "unknown" : d;
                    let column = `count(case when type="${d}" then type end) as "${asStr}"`;
                    return column;
                }).join(",");
                let whereStr = tableUtil.where([
                    tableUtil.condition.rangeDate("update_nuc", "day"),
                    tableUtil.condition.optionalSelectStr("type", "type")
                ]);
                let groupStr = tableUtil.group(["update_nuc"]);
                let sqlCommand = req.body.display == "统计" ?
                    `select update_nuc as day,count(av) as total,${typeStr} from log_nuclear.aom_error_info ${whereStr} ${groupStr}` :
                    `select update_nuc as day,av,chid,et,mt,nwt,ov,pgn,tm,type from log_nuclear.aom_error_info ${whereStr}`;
                return sqlCommand;
            },
            readCheck: ()=> {
                return tableUtil.check.rangeDay("day") && tableUtil.check.regex("display", /^(统计|查询)$/) && tableUtil.check.select("type");
            }
        },
        {
            id: "retentionAndLtv",
            ...
            read: ()=> {
                ...
                let whereArr = [
                    tableUtil.condition.optionalSelectNum("serverid", "server"),
                    tableUtil.condition.notEqual("serverid", 0),
                    tableUtil.condition.rangeDate(dayColumnStr, "day")
                ];
                let whereStr = tableUtil.where(whereArr);
                let groupStr = tableUtil.group(groupArr);
                ...
                let sqlCommand = `select ${tableUtil.column.optionalSelect("serverId", "server")}${dayColumnStr} as day,${dnu} as dnu,${retention(1)},${retention(2)},
            ...
            readCheck: ()=> {
                return tableUtil.check.regex("type", /^(角色|账号|设备)$/) && tableUtil.check.rangeDay("day") && tableUtil.check.select("server");
            }
        },
        {
            id: "level",
            ...
            read: ()=> {
                let whereArr = [
                    tableUtil.condition.optionalSelectNum("serverid", "server"),
                    tableUtil.condition.notEqual("serverid", 0),
                    tableUtil.condition.simpleStr("date", "day"),
                    tableUtil.condition.equal("channel", 0),
                    tableUtil.condition.equal("ptid", 0)
                ];
                ...
                let sqlCommand = `select ${tableUtil.column.optionalSelect("serverid", "server")}date as day,channel,ptid as client,
            ...
            readCheck: ()=> {
                return tableUtil.check.select("server") && tableUtil.check.day("day") && tableUtil.check.regex("levelSpacing", /^(1|5|10|15|20)$/);
            }
        },
        {
            id: "loginLevel",
            ...
            read: ()=> {
                ...
                let whereStr = tableUtil.where(whereArr);
                let groupStr = tableUtil.group(groupArr);
                let groupLevel = `((level - 1) div ${req.body.levelSpacing})`;
                let levelName = req.body.levelSpacing == 1 ? "level" :
                    `concat(${groupLevel}*${req.body.levelSpacing}+1,"-",${groupLevel}*${req.body.levelSpacing}+${req.body.levelSpacing})`;
                let sqlCommand = `select ${tableUtil.column.optionalSelect("serverId", "server")}date as day,
                                    ${groupLevel} as groupLevel,${levelName} as levelName,
                                        count(roleId) as num
                                            from log_nuclear.login_data 
                                                ${whereStr} ${groupStr}`;
        ...
        },
        {
            ...
            read: ()=> {
                ...
                let whereStr = tableUtil.where(whereArr);
                let groupStr = tableUtil.group(groupArr);
                let sqlCommand = `select ${tableUtil.column.optionalSelect("serverid", "server")}date as day,channel,ptid as client,vip as vipLevel,sum(num) as num,sum(all_num) as total
                                            from res.vip_ds 
                                                ${whereStr} ${groupStr}`;
                return sqlCommand;
            },
            readCheck: ()=> {
                return tableUtil.check.select("server") && tableUtil.check.day("day");
            }
        },
        {
            id: "diamond",
            ...
            read: ()=> {
                let whereStr = tableUtil.where([
                    tableUtil.condition.simpleStr("rank_type", "rankType"),
                    tableUtil.condition.simpleStr("date", "day"),
                    tableUtil.condition.optionalSelectNum("serverid", "server"),
                    tableUtil.condition.notEqual("serverid", 0)
                ]);
            ...
            readCheck: ()=> {
                return tableUtil.check.select("server") && tableUtil.check.day("day") && tableUtil.check.regex("rankType", /^(金钻|钻石)$/)
                    && tableUtil.check.regex("limit", /^\d+$/);
            }
        },
        {
            id: "snap",
            ...
            read: ()=> {
                let whereArr = [
                    tableUtil.condition.optionalInputStr("imeiId", "deviceId"),
                    tableUtil.condition.optionalInputStr("accountId", "accountId"),
                    tableUtil.condition.optionalInputStr("roleId", "roleId"),
                    tableUtil.condition.optionalInputStr("account", "account"),
                    tableUtil.condition.optionalSelectNum("serverid", "server"),
                    tableUtil.condition.notEqual("serverid", 0)
                ];
                if (req.body.roleQueryType == "精确匹配") {
                    whereArr.push(tableUtil.condition.optionalInputStr("role_name", "role"));
                } else {
                    whereArr.push(tableUtil.condition.optionalInputLikeStr("role_name", "role"));
                }
                let whereStr = tableUtil.where(whereArr);
            ...
            readCheck: ()=> {
                return tableUtil.check.optionalRegex("deviceId", /^\d+$/) && tableUtil.check.optionalRegex("accountId", /^\d+$/)
                    && tableUtil.check.optionalRegex("roleId", /^\d+$/) && tableUtil.check.optionalRegex("role", /^[^ ]+$/)
                    && tableUtil.check.optionalRegex("account", /^[^ ]+$/) && tableUtil.check.regex("roleQueryType", /^(精确匹配|模糊查询)$/)
                    && tableUtil.check.select("server");
            }
        },
        {
            id: "chargeRank",
            ...
            read: ()=> {
                ...
                let sqlCommand = `select serverId as server,roleId,role_name as role,os,role_createDate as roleCreateDate,role_tianshu as roleTianshu,
                                    sum(pay) as money,sum(pay_count) as times,date as day from log_nuclear.pay_data 
                                        ${whereStr} ${tableUtil.group(["server", "roleId"])}`;
                return sqlCommand;
            },
            readCheck: ()=> {
                return tableUtil.check.select("server") && tableUtil.check.day("day");
        ...
        },
        {
            ...
            columns: [
                ...
                {id: "node5", name: "无法连接网关", checked: true},
                {id: "node6", name: "连接网关返回错误", checked: true},
                {id: "node7", name: "资源包开始更新", checked: true},
                {id: "node8", name: "资源下载成功，资源号", checked: true},
                {id: "node9", name: "资源下载失败，资源号", checked: true},
                {id: "node10", name: "资源包更新失败", checked: true},
                {id: "node11", name: "资源包更新完成", checked: true},
                {id: "node12", name: "补丁包开始更新", checked: true},
                {id: "node13", name: "补丁包更新失败", checked: true},
                {id: "node14", name: "补丁包更新完成", checked: true},
                {id: "node15", name: "所有更新完成", checked: true},
                {id: "node16", name: "进入logo主界面", checked: true},
                {id: "node17", name: "弹出账号登陆框", checked: true},
                {id: "node18", name: "弹出自登陆界面", checked: true},
                {id: "node19", name: "弹出第三方登录界面FB", checked: true},
                {id: "node20", name: "弹出第三方登录界面VK", checked: true},
                {id: "node21", name: "快速登录", checked: true},
                {id: "node22", name: "选服界面", checked: true},
                {id: "node23", name: "进入游戏", checked: true},
                {id: "node24", name: "开场战斗开始", checked: true},
                {id: "node25", name: "开场战斗结束", checked: true},
                {id: "node26", name: "点击输入角色名字", checked: true},
                {id: "node27", name: "关闭输入角色名字", checked: true},
                {id: "node28", name: "创建角色", checked: true},
                {id: "region", name: "区域", checked: true, tdStyle: {"white-space": "nowrap"}},
            ],
            extraFilter: [
            ...
            read: ()=> {
                let groupStr = req.body.type == "独立" ? tableUtil.group(["update_nuc", "region"]) : tableUtil.group(["region"]);
                let nodeArr = [
                    {event: "1000", scope: "1", name: "启动app"},
                    {event: "1001", scope: "1", name: "进入logo主界面"},
                    {event: "1002", scope: "1", name: "加载语言选框"},
                    {event: "1003", scope: "1", name: "选择语言"},
                    {event: "1020", scope: "1", name: "无法连接网关"},
                    {event: "1021", scope: "1", name: "连接网关返回错误"},
                    {event: "1004", scope: "1", name: "资源包开始更新"},
                    {event: "1022", scope: "1", name: "资源下载成功，资源号"},
                    {event: "1023", scope: "1", name: "资源下载失败，资源号"},
                    ...
                    {event: "1008", scope: "1", name: "关闭账号登陆框"},
                    ...
                    {event: "1024", scope: "1", name: "点击输入角色名字"},
                    {event: "1024", scope: "2", name: "关闭输入角色名字"},
                ...
                let whereStr = tableUtil.where([tableUtil.condition.rangeDate("update_nuc", "day")]);
            ...
            readCheck: ()=> {
                return tableUtil.check.rangeDay("day") && tableUtil.check.regex("type", /^(独立|汇总)$/);
            }
        },
    ];
};
